...
@section('title', 'Ajouter Employé - Co-op ERP')
...
<div style="padding: 1.5rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); margin: -2rem -2rem 2rem; border-radius: 8px; color: white;">
    <h1 style="font-size: 1.75rem; margin: 0; font-weight: bold;">➕ Nouvel Employé</h1>
    <p style="margin-top: 0.5rem; opacity: 0.9;">Fiche employé : nom, poste, contrat, salaire, contact, statut</p>
</div>
...
<div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); max-width: 800px;">
    <form action="{{ route('employes.store') }}" method="POST" style="display: flex; flex-direction: column; gap: 1.5rem;">
        @csrf
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div>
                ...
                <input type="text" name="poste" value="{{ old('poste') }}" placeholder="Ex: Responsable production"
                    style="width: 100%; padding: 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; box-sizing: border-box;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div>
                ...
                <input type="number" name="salaire" value="{{ old('salaire') }}" step="0.01" min="0" placeholder="0.00"
                    style="width: 100%; padding: 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; box-sizing: border-box;">
            ...
            <div>
                ...
                <select name="salaire_type" style="width: 100%; padding: 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; box-sizing: border-box;">
                    <option value="mensuel" {{ old('salaire_type', 'mensuel') === 'mensuel' ? 'selected' : '' }}>Mensuel</option>
                    <option value="journalier" {{ old('salaire_type') === 'journalier' ? 'selected' : '' }}>Journalier</option>
                    <option value="horaire" {{ old('salaire_type') === 'horaire' ? 'selected' : '' }}>Horaire</option>
                </select>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div>
                ...
                <input type="text" name="telephone" value="{{ old('telephone') }}" placeholder="+212 6 12 34 56 78"
                    style="width: 100%; padding: 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; box-sizing: border-box;">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" placeholder="employe@example.com"
                    style="width: 100%; padding: 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; box-sizing: border-box;">
        ...
        <div>
            ...
            <textarea name="adresse" rows="3" placeholder="Adresse complète"
                style="width: 100%; padding: 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; box-sizing: border-box; resize: vertical;">{{ old('adresse') }}</textarea>
        ...
        <div>
            ...
            <div style="display: flex; gap: 1.5rem;">
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="radio" name="statut" value="actif" {{ old('statut', 'actif') === 'actif' ? 'checked' : '' }}>
                    <span>Actif</span>
                </label>
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="radio" name="statut" value="inactif" {{ old('statut') === 'inactif' ? 'checked' : '' }}>
                    <span>Inactif</span>
                </label>
            </div>
